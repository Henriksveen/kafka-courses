...
class CallTopologyTest {

    private TopologyTestDriver testDriver;
    private TestInputTopic<NobillRecordKey, CallRecordValue> inputTopic;
    ...
    void setUp() {
        ...
        AppTopology.withBuilder(streamsBuilder, MockSerdes, AggregateType.CALL);
        ...
        inputTopic = testDriver.createInputTopic(AppConfig.NOBILL_CALL_RECORD_TOPIC,
                MockSerdes.<NobillRecordKey>Key().serializer(), MockSerdes.<CallRecordValue>Value().serializer());
        hourOutputTopic = testDriver.createOutputTopic(AppConfig.NOBILL_CALL_RECORD_HOUR_TOPIC,
                MockSerdes.<CallAggregateKey>Key().deserializer(), MockSerdes.<CallAggregateValue>Value().deserializer());
    ...
    @Test
    @DisplayName("Test the CALL topology flow from the source topic through hour to the final output topic.")
    ...
    @Test
    @DisplayName("CALL Topology should aggregate correctly HOUR")
    ...
    @Test
    @DisplayName("CALL Topology should aggregate correctly DAY")
    ...
    private void assertKeyValue(KeyValue<CallAggregateKey, CallAggregateValue> record, long aggTime, double cost, long calls, long duration) {
        assertAll(
                ...
                () -> assertEquals(calls, record.value.getCallCount()),
